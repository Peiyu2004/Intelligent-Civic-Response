...
- broken_streetlight
  ...
  = Look for: bent pole, light bulb, shattered lamp, dark unlit area at night. if there is no light bulb, it is not a streetlight.
  = ROAD BLOCKING RULE: If the pole has fallen ACROSS the road and is blocking traffic → score must be 8-10. If pole is fallen at roadside only → score 1-7. if the pole didnt block the road and the sign is still visible, give 1-5.
...
{
  "damage_type": "<PRIMARY category — the most dangerous issue>",
  "severity_score": <integer 1–10>,
  "confidence": "<high / medium / low>",
  "description": "<one clear sentence describing ALL visible damage, most serious first>",
  "urgency": "<immediate / high / medium / low>",
  "recommended_action": "<one sentence covering repair actions for ALL damage types found>",
  "secondary_issues": [
    {
      "damage_type": "<category>",
      "description": "<brief description of this specific issue>",
      "recommended_action": "<specific action for this issue>"
    }
  ]
}
...
EXAMPLE — cracked pavement + worn road markings visible:
{
  "damage_type": "cracked_pavement",
  "severity_score": 4,
  "confidence": "high",
  "description": "Visible cracks across the asphalt surface with heavily faded lane markings throughout the same section.",
  "urgency": "medium",
  "recommended_action": "Dispatch road crew to repair cracks and repaint lane markings in the same visit.",
  "secondary_issues": [
    {
      "damage_type": "worn_road_marking",
      "description": "Lane markings are heavily faded and barely visible alongside the cracked section.",
      "recommended_action": "Repaint all lane markings after pavement repair is complete."
    }
  ]
}
...
TYPE_WEIGHT = {
    ...
    "worn_road_marking":   0,   # general road hazard
...
def validate_and_fix(result: dict) -> dict:
    ...
    # Validate and clean secondary_issues
    if not isinstance(result.get("secondary_issues"), list):
    ...
    for issue in result["secondary_issues"]:
        if not isinstance(issue, dict):
            continue
        # Fix damage_type in secondary issues
        raw = str(issue.get("damage_type", "")).lower()
        if raw not in valid_damage_types:
            if "pothole" in raw or "hole" in raw:
                issue["damage_type"] = "pothole"
            elif "light" in raw or "lamp" in raw:
                issue["damage_type"] = "broken_streetlight"
            elif "crack" in raw or "pavement" in raw:
                issue["damage_type"] = "cracked_pavement"
            elif "flood" in raw or "drain" in raw or "water" in raw:
                issue["damage_type"] = "flooded_drain"
            elif "sign" in raw or "signage" in raw:
                issue["damage_type"] = "fallen_sign"
            elif "marking" in raw or "road" in raw or "worn" in raw:
                issue["damage_type"] = "worn_road_marking"
            elif "vandal" in raw or "graffiti" in raw:
                issue["damage_type"] = "vandalism"
            elif "debris" in raw or "block" in raw:
                issue["damage_type"] = "debris"
            else:
                issue["damage_type"] = "other"
        # Ensure required fields exist
        if not issue.get("description"):
            issue["description"] = "Additional damage observed."
        if not issue.get("recommended_action"):
            issue["recommended_action"] = "Inspect and repair as needed."
        # Don't duplicate the primary issue
        if issue["damage_type"] != result["damage_type"]:
            cleaned_secondary.append(issue)

    result["secondary_issues"] = cleaned_secondary
...
def generate_work_order(analysis: dict, report_id: str) -> dict:
    ...
    secondary = analysis.get("secondary_issues", [])
    ...
    work_order = {
        ...
        "PRIMARY_DAMAGE_TYPE": analysis["damage_type"].replace("_", " ").upper(),
        ...
        "TOTAL_ISSUES_FOUND":  1 + len(secondary),
        "SECONDARY_ISSUES": [
            {
                "DAMAGE_TYPE":     s["damage_type"].replace("_", " ").upper(),
...
def process_citizen_report(image_path: str, location: str, report_id: str = "WO-001") -> dict:
    ...
    print(f"   Primary Issue: {analysis['damage_type'].replace('_', ' ').upper()}")
    ...
    if secondary:
        print(f"\n   ⚠️  Additional Issues Detected ({len(secondary)}):")
        for i, issue in enumerate(secondary, 1):
            dtype = issue['damage_type'].replace('_', ' ').title()
            print(f"     {i}. {dtype}")
...
def mock_test():
    ...
    fake_analysis = {
        ...
        "secondary_issues": [
            {
                "damage_type": "cracked_pavement",
                "description": "Minor cracks visible on the road surface near the flooded drain.",
                "recommended_action": "Inspect and patch cracks after drain is cleared."
            }
        ]
    }
